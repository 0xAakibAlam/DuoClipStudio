...
function App() {
    const [trackA, setTrackA] = useState<Track>({ file: null, url: null, start: 0, duration: 5 });
    const [trackB, setTrackB] = useState<Track>({ file: null, url: null, start: 0, duration: 5 });
    ...
    const [timelinePixelWidth, setTimelinePixelWidth] = useState(800);
    ...
    const updateTimelineWidth = useCallback(() => {
        if (timelineAreaRef.current) {
            ...
            setTimelinePixelWidth(areaWidth);
        }
    }, []);
    ...
    }, [trackA, trackB, isPlaying, playhead, activeTrack]);
    ...
    const handleLoadVideo = (track: 'A' | 'B', file: File) => {
        ...
        video.onloadedmetadata = () => {
            ...
            if (track === 'A') {
                setTrackA({ file, url, start: 0, duration });
            } else {
                setTrackB({ file, url, start: 0, duration });
            }
        };
    };
    ...
    const handleDragEnd = (event: any) => {
        ...
        if (active.id === 'trackA' && trackA.url) {
            ...
            setTrackA({ ...trackA, start: newStart });
            ...
            setTrackB({ ...trackB, start: newStart });
        }
    };
